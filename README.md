...
- **Separación de responsabilidades**:
  - `data_loader.py`: lectura de CSV y manejo de errores
  - `utils.py`: validación/normalización de registros
  - `processor.py`: lógica de negocio (filtros, orden, estadísticas)
  - `exporter.py`: escritura de JSON
  - `main.py`: interacción del usuario (CLI)

- **Funciones testeables**:  
  La lógica central se implementa en funciones puras (sin `input()`), facilitando tests con `pytest`.

- **Manejo de errores**:
  - `FileNotFoundError` si el CSV no existe
  - validación de tipos y rangos antes de procesar
...
- Los datos de ejemplo están en `data/estudiantes.csv`.
- Para modificar el análisis (filtros o estadísticas), edita funciones en `processor.py`.

---

## Autor / Entrega

Repositorio publicado en GitHub como parte de los entregables del curso:
- Código modular
